				...
				<header class="heading-tab">
					<?php if( strlen($title) > 0 ): ?>
						<h3 class="heading-title <?php echo (empty($icon_image) && $icon_fontawesome == '')?'no-icon':'has-icon' ?> ">
							...
							<span class="ic-image">
							<?php
							$icon_url = wp_get_attachment_image_url($icon_image);
							if( $icon_url ){
							?>
								<img class="ts-lazy-load" src="data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%201%201'%3E%3C/svg%3E" data-src="<?php echo esc_url($icon_url); ?>" alt="<?php esc_attr_e('Icon', 'themesky'); ?>" />
							<?php
							}
							?>
							</span>
	...
	function ts_products_in_category_tabs_2_shortcode($atts, $content){
		...
		<div class="<?php echo esc_attr(implode(' ', $classes)); ?>" id="<?php echo esc_attr($rand_id) ?>" data-atts="<?php echo htmlentities(json_encode($atts)); ?>">
			...
			<div class="column-tabs">
				...
				<?php 
				$tab_icon_index = 0;
				$placeholder_image = "data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%201%201'%3E%3C/svg%3E";
				if( $show_general_tab ){
					$icon = $icon_hover = '';
					if( isset($tab_icons_arr[$tab_icon_index]) ){
						$icon_url = wp_get_attachment_image_url($tab_icons_arr[$tab_icon_index]);
						if( $icon_url ){
							$icon = '<img class="ts-lazy-load" src="'.$placeholder_image.'" data-src="'.esc_url($icon_url).'" alt="'.esc_attr__('Icon', 'themesky').'" />';
						}
						
						if( isset($tab_icons_hover_arr[$tab_icon_index]) ){
							$icon_url = wp_get_attachment_image_url($tab_icons_hover_arr[$tab_icon_index]);
							if( $icon_url ){
								$icon_hover = '<img class="ts-lazy-load hover-icon" src="'.$placeholder_image.'" data-src="'.esc_url($icon_url).'" alt="'.esc_attr__('Icon', 'themesky').'" />';
							}
						}
					}
					$tab_icon_index++;
